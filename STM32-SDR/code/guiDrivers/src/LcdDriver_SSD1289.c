...
/*
 * Code for LCD Driver for SSD1289 chip
 *
...
void	Graph_Array_Line ( int16_t* array, int16_t divisor ){

	int16_t x, y, x_old = 0, y_old = 0;
...
//		y = ( ( *array++ ) / divisor ) + 120;
		y = ( ( *( array += 4 ) ) / divisor ) + 120;

		if ( x == 0 ) y_old = y;

		LCD_DrawUniLine ( x_old, y_old, x, y );
...
void	Graph_Spectrum ( int16_t* array ){

	int16_t x, y, max, index, divisor;
	...
	arm_max_q15 ( array, 320, &max, &index );
...
//		y = ( ( * ( array += 2 ) ) / divisor );
		...
		LCD_DrawUniLine ( x, 205, x, 205 - y );

		if ( x % 64 == 0 ) LCD_DrawUniLine ( x, 205, x, 215 );

		if ( x % 32 == 0 ) LCD_DrawUniLine ( x, 205, x, 210 );

		}

	LCD_StringLine( 64*1-12,220, "500");

	LCD_StringLine( 64*2-16,220, "1000");

	LCD_StringLine( 64*3-16,220, "1500");

	LCD_StringLine( 64*4-16,220, "2000");

//	LCD_StringLine( 64*5-32,215, "2500");
...
{
  LCD_WriteReg(LCD_REG_78, Ypos);
  LCD_WriteReg(LCD_REG_79, 319-Xpos);
}
...
void LCD_Integer ( int16_t number, uint16_t x, uint8_t y )	{
	...
	while ( number > 0 ) {
		...
		if ( ( index == 6 || index == 2 ) && number > 0 ) buffer [ index-- ] = ',';

		}

	if ( sign == 1 ) buffer [ index ] = '-';
