...
void LCD_DrawFFT(void)
{
	...
	for (j=0;j<60;j++) Pixel((int)temp+40,179+j,RED);
}
...
void LCD_BackLight(int procentai)
{
  if (procentai>100)
    ...
    {procentai=0;}
  Channel1Pulse =(uint16_t)(((uint32_t)procentai*(TimerPeriod-1))/100);
