...
void Draw_Touch_Point(uint16_t x,uint16_t y)
{
	...
	if ((X_Point>320) && (Y_Point>192))	{  //TX Selection
		...
		switch(Mode) {
		case 0: Xmit_SSB_Sequence();
		break;
		case 1: Xmit_CW_Sequence();
		break;
		case 2: Xmit_PSK_Sequence();
		break;
		}  // End of switch
	...
	if ((X_Point>144) && (X_Point<176) && (Y_Point<20)&& (Tx_Flag==0))	{  //CW Mode Selection
	Mode = 1;
	Set_Mode_Display();
	}

	if ((X_Point>188) && (X_Point<228) && (Y_Point<20)&& (Tx_Flag==0))	{  //PSK Mode Selection
	Mode = 2;
	Set_Mode_Display();
	}
