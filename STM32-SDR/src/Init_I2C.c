...
uint32_t I2C_WriteRegister(uint8_t DeviceAddr, uint8_t RegisterAddr, uint8_t RegisterValue)
   ...
   {
     if((Timeout--) == 0) return 1; //TIMEOUT_UserCallback();
   }
   ...
   {
     if((Timeout--) == 0) return 2; //TIMEOUT_UserCallback();
   }
   ...
   {
     if((Timeout--) == 0) return 3; // Codec_TIMEOUT_UserCallback();
   }
   ...
   {
     if((Timeout--) == 0) return 4; // Codec_TIMEOUT_UserCallback();
   }
   ...
   {
     if((Timeout--) == 0) return 5; // Codec_TIMEOUT_UserCallback();
   }
...
uint32_t I2C_ReadSlave( uint8_t Address, uint8_t Register)
    {
      ...
      while(I2C_GetFlagStatus(I2C1, I2C_FLAG_BUSY))
      {
        if((Timeout--) == 0) return 1; //Codec_TIMEOUT_UserCallback();
      }
      ...
      while (!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_MODE_SELECT))
      {
        if((Timeout--) == 0) return 2; //Codec_TIMEOUT_UserCallback();
      }
      ...
      {
        if((Timeout--) == 0) return 6; //Codec_TIMEOUT_UserCallback();
      }
      ...
      {
        if((Timeout--) == 0) return 7;  //Codec_TIMEOUT_UserCallback();
      }
      ...
      {
        if((Timeout--) == 0) return 8; //Codec_TIMEOUT_UserCallback();
      }
...
uint32_t I2C_ReadEEProm( uint16_t RegisterAddr)
    {
      ...
      while(I2C1->CR1 & I2C_CR1_STOP)
      {
        if((Timeout--) == 0) return 9; //Codec_TIMEOUT_UserCallback();
      }
